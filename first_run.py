...
def main():
    ...
    if not admin:
        user.objects.create_superuser('admin', 'admin@myproject.com', 'admin')  # ToDo for pipeline
    ...
    contract1 = Contract.objects.get_or_create(name=f"{trans['Contract'][lang]} #1", project=project[0], company=company2[0], contract_type=buy[0], amount_netto=-100, vat=19, amount_brutto=-119)
    contract2 = Contract.objects.get_or_create(name=f"{trans['Contract'][lang]} #2", project=project[0], company=company3[0], contract_type=sale[0], amount_netto=200, vat=19, amount_brutto=238)

    Bill = apps.get_model("Baumanagement", "Bill")
    Bill.objects.get_or_create(name=f"{trans['Bill'][lang]} #1.1", contract=contract1[0], amount_netto=-50, vat=19, amount_brutto=-50*1.19)
    Bill.objects.get_or_create(name=f"{trans['Bill'][lang]} #1.2", contract=contract1[0], amount_netto=-25, vat=19, amount_brutto=-25*1.19)
    Bill.objects.get_or_create(name=f"{trans['Bill'][lang]} #2.1", contract=contract2[0], amount_netto=100, vat=19, amount_brutto=100*1.19)
    Bill.objects.get_or_create(name=f"{trans['Bill'][lang]} #2.2", contract=contract2[0], amount_netto=50, vat=19, amount_brutto=50*1.19)

    Payment = apps.get_model("Baumanagement", "Payment")
    Payment.objects.get_or_create(name=f"{trans['Payment'][lang]} #1.1", contract=contract1[0], amount_netto=-25, vat=19, amount_brutto=-25*1.19)
    Payment.objects.get_or_create(name=f"{trans['Payment'][lang]} #2.1", contract=contract2[0], amount_netto=50, vat=19, amount_brutto=50*1.19)
